...
export default function LeadManagementSystem() {
  ...
  const downloadExcel = () => {
    ...
    link.setAttribute("href", url); link.setAttribute("download", `Leads_${new Date().toLocaleDateString()}.csv`)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f4f4f4]">
        <div className="bg-white p-10 w-full max-w-md border-t-4 border-[#0f62fe] shadow-sm">
          ...
          <input className="w-full bg-[#f4f4f4] border-b border-[#8d8d8d] p-3 mb-4 outline-none focus:border-[#0f62fe]" placeholder="ID" onChange={e => setLoginId(e.target.value)} />
          <input className="w-full bg-[#f4f4f4] border-b border-[#8d8d8d] p-3 mb-8 outline-none focus:border-[#0f62fe]" type="password" placeholder="PW" onChange={e => setPassword(e.target.value)} />
      ...
      <main className="p-8 max-w-[1200px] mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            <div className="flex gap-2 w-full md:w-auto">
                <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-white border border-[#8d8d8d] px-4 py-2 text-sm outline-none focus:border-[#0f62fe] flex-1 md:w-64" placeholder="이름/연락처 검색..." />
                <button onClick={downloadExcel} className="bg-[#161616] text-white px-4 py-2 text-sm font-bold hover:bg-black transition flex items-center gap-2">📥 Export</button>
            </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-[1px] bg-[#e0e0e0] border border-[#e0e0e0] mb-8 shadow-sm">
            ...
            {/* --- 오늘 수신/배정 건수 --- */}
            <div className="bg-white p-6">
                <p className="text-[11px] font-bold text-[#0f62fe] uppercase mb-1 tracking-widest">Today New</p>
                <p className="text-4xl font-light text-[#0f62fe]">
                    {isAdmin 
                      ? leads.filter(l => isToday(l.created_at)).length 
                      : leads.filter(l => isToday(l.assigned_at)).length
                    }
                </p>
            </div>
            <div className="bg-white p-6">
                <p className="text-[11px] font-bold text-[#a2191f] uppercase mb-1 tracking-widest">Pending</p>
                <p className="text-4xl font-light text-[#a2191f]">{leads.filter(l => l.status === '상담중' || l.status === '재통화필요').length}</p>
            </div>
            <div className="bg-white p-6">
                <p className="text-[11px] font-bold text-[#198038] uppercase mb-1 tracking-widest">Closed</p>
        ...
        <div className="space-y-[1px] bg-[#e0e0e0] border border-[#e0e0e0] shadow-sm">
              ...
              <div className="w-full md:w-[320px] bg-[#fbfbfb] p-6 border-l border-[#e0e0e0] flex flex-col justify-center">
                {isAdmin ? (
                  <div className="space-y-3">
                    ...
                    <button onClick={() => assignLead(lead.id)} className="w-full bg-[#161616] text-white p-2.5 text-xs font-bold hover:bg-black">Confirm</button>
                  ...
                  <div className="space-y-4">
                    ...
                    <textarea id={`mm-${lead.id}`} className="w-full bg-white border border-[#8d8d8d] p-2.5 text-xs h-24 outline-none resize-none focus:border-[#0f62fe]" placeholder="Notes..." />
                    <button 
                      onClick={() => {
          ...
          {filteredLeads.length === 0 && (
            <div className="bg-white p-20 text-center text-slate-300 italic">검색 결과가 없습니다.</div>
          )}
        </div>
      </main>
    </div>
